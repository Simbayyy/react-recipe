...
import { RecipeType, Ingredient as IngredientType, RecipeSchema}  from "../functions/types"
import {Ingredient} from "./Ingredient"
import React, { useEffect } from "react"

const Recipe: React.FunctionComponent<Record<string,never>> =  (): React.ReactElement => {
    ...
    function displayRecipe(recipe: undefined | RecipeSchema) {
        if (recipe) {
            return <div className="content__recipes__recipe">
                ...
                <a href={recipe.url} target="_blank" rel="noreferrer" className="content__recipes__recipe__link">{recipe.url}</a>
                <div className="content__recipes__recipe__summary">
                    {(recipe.recipeIngredient || []).length} ingrédient{(recipe.recipeIngredient || []).length > 1 ? "s": ""}, {`${recipe.totalTime} ${recipe.totalTime}`}
                </div>
                {renderIngredients(recipe.recipeIngredient as IngredientType[] || [])}
             </div>
        } else {
